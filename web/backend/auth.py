"""Session token store -- ported verbatim from VOD & DVR Manager's auth.py.
No VOD-specific logic in the original, so nothing to adapt."""
import json
import secrets
import time
...
def revoke_all_sessions(except_token: str | None = None) -> None:
    ...
    TTL just because the password that guarded it was rotated. except_token
    keeps the caller's own current session alive so changing your own
