...
import DeploymentProfilesEditor from '@/components/weather/DeploymentProfilesEditor'
import { DEFAULT_STREAM_PROFILE_NAME } from '@/components/weather/channelConstants'
import api from '@/lib/api'
import type {
  BulkDeployResult, ChannelGroup, DispatcharrChannelProfile,
  DispatcharrConnection, StreamProfile, WebChannel,
} from '@/types'

// Near-duplicate of weather/DeployModal.tsx against /api/webchannels/
// instead of /api/channels/ -- deliberately a separate copy, not a shared
// generic component, matching this whole feature's own convention of never
// making the weather-channel code path depend on anything web-channel-
// specific (see web_channel_routes.py's module docstring for the backend
// side of the same reasoning). DeploymentProfilesEditor IS reused as-is --
// it's already generic (channelId + connectionId + a fixed API shape), not
// weather-specific despite living under components/weather/.
export default function WebChannelDeployModal({ channel, onClose }: { channel: WebChannel; onClose: () => void }) {
  ...
  const { data: groups, isLoading: groupsLoading } = useQuery<ChannelGroup[]>({
    ...
    queryFn: () => api.get(`/dispatcharr-connections/${connectionId}/groups/`).then((r) => r.data),
  ...
  const deployMutation = useMutation({
    mutationFn: () =>
      api.post(`/webchannels/${channel.id}/deploy/`, {
  ...
  const undeployMutation = useMutation({
    mutationFn: (targetConnectionId: number) => api.delete(`/webchannels/${channel.id}/deploy/${targetConnectionId}/`),
  ...
  const refreshMutation = useMutation({
    mutationFn: (targetConnectionId: number) => api.post(`/webchannels/${channel.id}/deploy/${targetConnectionId}/refresh/`),
  ...
  const bulkDeployMutation = useMutation({
    mutationFn: () =>
      api.post(`/webchannels/${channel.id}/deploy-bulk/`, {
    ...
    >
      <div className="w-full max-w-md rounded-lg border border-border bg-card shadow-xl p-4 space-y-3 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <p className="text-sm font-medium">Deploy "{channel.channel_name}"</p>
            ...
            {mode === 'single' ? (
              <>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    "Redirect" (recommended) sends the player straight to our own stream. Needs a real public URL
                    (Settings page), not a private-only address. "Proxy" adds Dispatcharr's own buffering.
                  </span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] text-muted-foreground">Logo URL (optional)</span>
                  <Input
                    className="h-8 text-xs" placeholder="https://.../icon.png"
              ...
              <>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <Input
                    className="h-8 text-xs" placeholder="e.g. Web Channels" list="bulk-webchannel-group-names"
                    value={bulkGroupName} onChange={(e) => setBulkGroupName(e.target.value)}
                  />
                  <datalist id="bulk-webchannel-group-names">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <Input
                    ...
                    list="bulk-webchannel-profile-names"
                    value={bulkProfileName}
                    onChange={(e) => { setBulkProfileName(e.target.value); setBulkProfileTouched(true) }}
                  />
                  <datalist id="bulk-webchannel-profile-names">
                    {bulkProfileNames.map((n) => <option key={n} value={n} />)}
                  </datalist>
                  <span className="text-[10px] text-muted-foreground">
                    Matched by name on each connection; leave blank to use each connection's own default.
